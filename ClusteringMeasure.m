function [result, Con] = ClusteringMeasure(Y, predY)
...
% ARI
[ARI,RI,~,~]  = RandIndex(Y,res);
% F Precision Recall
[f, p, r] = compute_f(Y,res);
% Contingency
Con = Contingency(Y, res); 



result = [ACC f RI]; % [7: ACC MIhat Purity ARI F-score Precision Recall]
% Contingency;
